// Stockyard Spur — API mock server.
// Define endpoints and responses, serve fake data instantly. Self-hosted.
...
import (
	...
	"github.com/stockyard-dev/stockyard-spur/internal/license"
	"github.com/stockyard-dev/stockyard-spur/internal/server"
...
func main() {
	...
	licenseKey := os.Getenv("SPUR_LICENSE_KEY")
	licInfo, licErr := license.Validate(licenseKey, "spur")
	...
	if licInfo != nil && licInfo.IsPro() {
		...
		log.Printf("  License:   Free tier (set SPUR_LICENSE_KEY to unlock Pro)")
	}
	...
	log.Printf("")
	log.Printf("  Stockyard Spur %s", version)
	log.Printf("  Mock base:      http://localhost:%d/mock/", port)
	...
	go func() {
		for {
			...
			if err != nil {
				...
				log.Printf("[cleanup] deleted %d old request log entries", n)
			}
		}
	}()
