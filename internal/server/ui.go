...
<div class="hdr"><h1><span>&#9670;</span> SPUR</h1><button class="btn btn-p" onclick="openForm()">+ Add</button></div>
...
async function load(){var r=await fetch(A+'/hooks').then(function(r){return r.json()});items=r.hooks||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.event||'').toLowerCase().includes(q)||(i.target_url||'').toLowerCase().includes(q)||(i.secret||'').toLowerCase().includes(q)});
...
if(i.event)h+='<div class="item-sub">'+esc(i.event)+'</div>';
...
if(i.target_url)h+='<span>'+esc(i.target_url)+'</span>';
if(i.secret)h+='<span>'+esc(i.secret)+'</span>';
...
function formHTML(item){var i=item||{name:"",event:"",target_url:"",secret:"",enabled:"",delivery_count:"",fail_count:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SPUR</h2>';
h+='<div class="fr"><label>Name *</label><input id="f-name" value="'+esc(i.name)+'"></div>';
h+='<div class="fr"><label>Event</label><input id="f-event" value="'+esc(i.event)+'"></div>';
h+='<div class="fr"><label>Target U R L</label><input id="f-target-url" value="'+esc(i.target_url)+'"></div>';
h+='<div class="fr"><label>Secret</label><input id="f-secret" value="'+esc(i.secret)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),event:document.getElementById('f-event').value.trim(),target_url:document.getElementById('f-target-url').value.trim(),secret:document.getElementById('f-secret').value.trim(),enabled:document.getElementById('f-enabled').value.trim(),delivery_count:document.getElementById('f-delivery-count').value.trim(),fail_count:document.getElementById('f-fail-count').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/hooks/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/hooks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
